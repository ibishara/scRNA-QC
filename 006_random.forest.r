...
# nGenes <- 25
        ...
        stTestList = splitCommon(sampTab = meta.sub, ncells = ncells, dLevel = var2)  # not enough 400 cells/class to sample high read cut-off
        stTest = stTestList[[1]]
        ...
        system.time(class_info <- scn_train(stTrain = stTrain, expTrain = expTrain, 
                                ...
                                dLevel = var2, colName_samp = "Cell.ID"))
        ...
        tm_heldoutassessment = assess_comm(ct_scores = classRes_val_all, stTrain = stTrain, stQuery = stTest, 
                                        dLevelSID = "Cell.ID", classTrain = var2, classQuery = var2, nRand = 50)
