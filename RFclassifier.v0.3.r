...
RF_run <- function (class, method) {
    ...
    # SCN prediction
    classRes_val_all = scn_predict(cnProc=class_info[['cnProc']], expDat = expTest, nrand = 50)  # Removed rand # number of training and validation cells must be equal. genes in model must be in validation set. | issue: some dropped genes lead to error
    ...
    classRes_val_all_noRand <- as.data.frame(classRes_val_all[!(row.names(classRes_val_all) %in% 'rand'), 1:(ncol(classRes_val_all)-50) ] )# remove rand category
    classRes_val_labels <- unlist(apply(classRes_val_all_noRand, MARGIN = 2, function(x) { x <- names(which(x == max(x))) })) # generate labels based off highest probabilities (excluding Random)
    ...
    #     print('classRes_val_labels is list')
    # } else {
    #     AUC.pROC <- multiclass.roc(stTest[, class], factor(classRes_val_labels, ordered = TRUE))$auc[1]
    #     print('classRes_val_labels is NOT list')}
    print(paste('AUC =', AUC.pROC))
    # changed from : 
    #     AUC.pROC <- multiclass.roc(stTest[, class], factor(classRes_val_labels[1,], ordered = TRUE))$auc[1]
    #     }
    # AUC.pROC <- multiclass.roc(stTest[, class], factor(classRes_val_labels, ordered = TRUE))$auc[1]
